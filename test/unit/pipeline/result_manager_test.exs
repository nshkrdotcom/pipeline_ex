defmodule Pipeline.ResultManagerTest do
  ...
  describe "extract_field/3" do
    ...
    test "extracts nested fields from results" do
      ...
      assert {:ok, 95} =
               ResultManager.extract_field(updated_manager, "step1", "data.analysis.score")

      assert {:ok, "Good quality"} =
               ResultManager.extract_field(updated_manager, "step1", "data.analysis.summary")
    end
    ...
    test "returns error for missing fields" do
      ...
      assert {:error, :field_not_found} =
               ResultManager.extract_field(updated_manager, "step1", "missing_field")
    end
  end
  ...
  describe "transform_for_prompt/3" do
    ...
    test "transforms content results for prompts" do
      ...
      assert {:ok, "Analysis result"} =
               ResultManager.transform_for_prompt(updated_manager, "step1")
    end
    ...
    test "formats JSON for prompts when requested" do
      ...
      assert {:ok, json_result} =
               ResultManager.transform_for_prompt(updated_manager, "step1", format: :json)
    ...
    test "extracts specific fields for prompts" do
      ...
      assert {:ok, "Extra info"} =
               ResultManager.transform_for_prompt(updated_manager, "step1", field: "metadata")
    end
  end
  ...
  describe "get_summary/1" do
    test "provides summary statistics" do
      ...
      manager =
        ResultManager.store_result(manager, "step1", %{text: "Success", success: true, cost: 0.01})

      # Add failed result
      manager =
        ResultManager.store_result(manager, "step2", %{
          text: "Failed",
          success: false,
          cost: 0.005
        })
