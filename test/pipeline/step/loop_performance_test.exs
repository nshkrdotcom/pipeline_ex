defmodule Pipeline.Step.LoopPerformanceTest do
  ...
  describe "Performance Tests" do
    test "handles large dataset with empty steps (structure test)" do
      ...
      step = %{
        ...
        # Empty steps to test loop structure only
        "steps" => []
      }
      ...
      # Should complete within 1 second for empty steps
  ...
  describe "Structure Tests" do
    test "validates nested loop context creation" do
      ...
      parent_context =
        Pipeline.Step.Loop.create_nested_loop_context("parent", %{"id" => 1}, 0, 1, context)
      ...
      child_context =
        Pipeline.Step.Loop.create_nested_loop_context("child", %{"name" => "child1"}, 0, 2, %{
          context
          | results: Map.merge(context.results, parent_context)
        })
