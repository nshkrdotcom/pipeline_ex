defmodule Pipeline.Error.NestedPipelineTest do
  ...
  describe "create_debug_log_entry/3" do
    test "creates comprehensive debug log entry" do
      ...
      assert result.total_steps == 0  # RecursionGuard.count_total_steps with mock context
    ...
    test "classifies different error types correctly" do
      ...
      circular_result = NestedPipeline.create_debug_log_entry("Circular dependency detected", context)
  ...
  describe "error message extraction" do
    test "extracts message from various error formats" do
      # Test string error
      assert NestedPipeline.create_debug_log_entry("Simple error", %{pipeline_id: "test"}).error_message == "Simple error"
      
      # Test error tuple
      assert NestedPipeline.create_debug_log_entry({:error, "Tuple error"}, %{pipeline_id: "test"}).error_message == "Tuple error"
      
      # Test error map
      assert NestedPipeline.create_debug_log_entry({:error, %{message: "Map error"}}, %{pipeline_id: "test"}).error_message == "Map error"
      
      # Test exception-like structure
      assert NestedPipeline.create_debug_log_entry(%{message: "Exception error"}, %{pipeline_id: "test"}).error_message == "Exception error"
    end
  end
  ...
  describe "context summarization" do
    test "summarizes context information correctly" do
      ...
      assert summary.context_size == 6  # Number of fields in context map
