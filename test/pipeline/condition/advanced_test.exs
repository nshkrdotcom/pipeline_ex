defmodule Pipeline.Condition.AdvancedTest do
  ...
  describe "mathematical expressions" do
    test "basic arithmetic operations" do
      ...
      assert Engine.evaluate("step1.score / 0 == 0", context) == true
    ...
    test "complex mathematical comparisons" do
      context = %{
        results: %{"analysis" => %{"score" => 8.5, "confidence" => 0.85, "threshold" => 0.8}}
      }

      assert Engine.evaluate("analysis.score * analysis.confidence > analysis.threshold", context) ==
               true
  ...
  describe "string functions" do
    test "contains function" do
      context = %{
        results: %{"step1" => %{"message" => "Hello World", "tags" => ["error", "warning"]}}
      }
    ...
    test "matches function" do
      context = %{
        results: %{"analysis" => %{"file_path" => "/src/test.ex", "email" => "user@example.com"}}
      }
  ...
  describe "date/time functions" do
    ...
    test "duration functions" do
      ...
      # 1 day = 86400 seconds
      assert Engine.evaluate("days(1) == 86400", context) == true
      # 1 hour = 3600 seconds
      assert Engine.evaluate("hours(1) == 3600", context) == true
      # 1 minute = 60 seconds
      assert Engine.evaluate("minutes(1) == 60", context) == true
    end
  end
