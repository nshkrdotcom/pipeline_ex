defmodule Pipeline.Codebase.QueryEngineTest do
  ...
  describe "find_related_files/2" do
    ...
    test "finds test files for source file", %{context: context} do
      related = QueryEngine.find_related_files(context, [
        file: "lib/user.ex",
        types: ["test"]
      ])
  ...
  describe "analyze_impact/2" do
    ...
    test "includes test files in impact analysis", %{context: context} do
      impact = QueryEngine.analyze_impact(context, [
        file: "lib/user.ex",
        include_tests: true
      ])
    ...
    test "respects max_depth parameter", %{context: context} do
      impact1 = QueryEngine.analyze_impact(context, [
        file: "lib/user.ex",
        max_depth: 1
      ])
      
      impact2 = QueryEngine.analyze_impact(context, [
        file: "lib/user.ex",
        max_depth: 3
      ])
  ...
  defp create_test_files do
    ...
    """
    File.write!(Path.join([@test_workspace_dir, "test", "account_test.exs"]), account_test_content)
