  ...
  def execute(step, orch) do
    ...
    result = case step[:functions] do
      nil ->
        ...
        case call_instructor_lite(prompt, Pipeline.Schemas.AnalysisResponse, model) do
          {:ok, response} ->
            ...
            Debug.log(orch.debug_log, "Gemini response (took #{elapsed / 1000}s):\n#{inspect(response)}\n")
      ...
      functions when is_list(functions) ->
        ...
        tool_schema = Pipeline.Tools.Adapters.InstructorLiteAdapter.create_function_schema(functions)
        ...
        case call_instructor_lite_with_tools(prompt, tool_schema, model) do
          {:ok, response} ->
            ...
            Debug.log(orch.debug_log, "Gemini response with tool calls (took #{elapsed / 1000}s):\n#{inspect(response)}\n")
