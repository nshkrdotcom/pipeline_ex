defmodule Pipeline.Step.CodebaseQuery do
  ...
  defp get_codebase_context(step, context) do
    workspace_dir = step["workspace_dir"] || Map.get(context, :workspace_dir) || Map.get(context, "workspace_dir") || "."
  ...
  defp execute_single_query(query_config, codebase_context, pipeline_context) when is_map(query_config) do
    ...
    cond do
      ...
      Map.has_key?(query_config, "find_dependencies") ->
        execute_find_dependencies_query(query_config["find_dependencies"], codebase_context, pipeline_context)
      
      Map.has_key?(query_config, "find_functions") ->
        execute_find_functions_query(query_config["find_functions"], codebase_context, pipeline_context)
      
      Map.has_key?(query_config, "find_related") ->
        execute_find_related_query(query_config["find_related"], codebase_context, pipeline_context)
      
      Map.has_key?(query_config, "analyze_impact") ->
        execute_analyze_impact_query(query_config["analyze_impact"], codebase_context, pipeline_context)
  ...
  defp execute_find_files_query(criteria, codebase_context, pipeline_context) when is_list(criteria) do
  ...
  defp execute_find_dependencies_query(config, codebase_context, pipeline_context) when is_list(config) do
  ...
  defp execute_find_functions_query(config, codebase_context, pipeline_context) when is_list(config) do
    resolved_config = resolve_criteria(config, pipeline_context)
  ...
  defp execute_find_related_query(config, codebase_context, pipeline_context) when is_list(config) do
    resolved_config = resolve_criteria(config, pipeline_context)
  ...
  defp execute_analyze_impact_query(config, codebase_context, pipeline_context) when is_list(config) do
    resolved_config = resolve_criteria(config, pipeline_context)
