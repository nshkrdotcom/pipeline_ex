    ...
    case find_matching_pattern(prompt) do
      ...
      :not_found ->
        ...
        case find_function_response(prompt) do
          {:ok, function_response} -> {:ok, function_response}
          :not_found ->
      ...
      "simple test" ->
        ...
        response = if options[:tools] && length(options[:tools]) > 0 do
          function_calls = Enum.map(options["tools"], fn tool ->
            %{
