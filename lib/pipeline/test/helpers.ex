defmodule Pipeline.Test.Helpers do
  ...
  def with_temp_dir(fun) do
    temp_dir =
      System.tmp_dir!()
      |> Path.join("pipeline_test_#{:rand.uniform(999_999)}")
  ...
  def create_gemini_step(opts) do
    ...
    step =
      if token_budget = Keyword.get(opts, :token_budget) do
    ...
    step =
      if output_file = Keyword.get(opts, :output_to_file) do
  ...
  def create_claude_step(opts) do
    ...
    step =
      if claude_options = Keyword.get(opts, :claude_options) do
  ...
  def create_temp_config_file(config_map) do
    temp_file =
      System.tmp_dir!()
      |> Path.join("test_config_#{:rand.uniform(999_999)}.yaml")
