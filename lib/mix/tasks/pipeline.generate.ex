defmodule Mix.Tasks.Pipeline.Generate do
    ...
    if File.exists?(full_path) and TestMode.live_mode?() do
      ...
      case Pipeline.Config.load_workflow(genesis_path) do
        {:ok, config} ->
          ...
          config_with_replaced_templates =
            replace_template_variables(config_with_input, merged_input)
  ...
  defp parse_package_result(package_result) when is_binary(package_result) do
    ...
    Mix.shell().info(
      "🐛 Debug - Binary package preview: #{String.slice(package_result, 0, 200)}..."
    )
  ...
  defp parse_validation_result(validation_result) when is_map(validation_result) do
    Mix.shell().info(
      "🐛 Debug - Validation result map keys: #{inspect(Map.keys(validation_result))}"
    )

    Mix.shell().info(
      "🐛 Debug - Validation result preview: #{inspect(validation_result, limit: 500)}"
    )
  ...
  defp parse_validation_result(validation_result) when is_binary(validation_result) do
    Mix.shell().info(
      "🐛 Debug - Binary validation result length: #{String.length(validation_result)}"
    )

    Mix.shell().info(
      "🐛 Debug - Binary validation preview: #{String.slice(validation_result, 0, 200)}..."
    )
  ...
  defp parse_simple_pipeline_result(pipeline_result) when is_map(pipeline_result) do
    Mix.shell().info(
      "🐛 Debug - Simple pipeline result map keys: #{inspect(Map.keys(pipeline_result))}"
    )

    Mix.shell().info(
      "🐛 Debug - Simple pipeline result preview: #{inspect(pipeline_result, limit: 500)}"
    )
    ...
    content =
      case pipeline_result do
        ...
        _ ->
          Mix.shell().info(
            "🐛 Debug - No text field found in simple pipeline result, using inspect"
          )
  ...
  defp parse_simple_pipeline_result(pipeline_result) when is_binary(pipeline_result) do
    Mix.shell().info(
      "🐛 Debug - Binary simple pipeline result length: #{String.length(pipeline_result)}"
    )

    Mix.shell().info(
      "🐛 Debug - Binary simple pipeline preview: #{String.slice(pipeline_result, 0, 200)}..."
    )
  ...
  defp extract_from_steps(steps) when is_map(steps) do
    ...
    result =
      Enum.find_value(step_names, fn step_name ->
